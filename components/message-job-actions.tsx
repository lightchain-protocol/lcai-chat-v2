...
export function MessageJobActions({
  ...
  const [disputeWindowLabel, setDisputeWindowLabel] = useState<string | null>(
    null
  );
  ...
  if (
    ...
    const openDisputeDialog = async () => {
      ...
      try {
        ...
        setDisputeWindowLabel(formatDurationLabel(Number(disputeWindow)));
          ...
          <AlertDialogContent className="sm:rounded-3xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                filed within{" "}
                {disputeWindowLabel ?? "the on-chain dispute window"} of job
                completion.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
/** Human copy for an on-chain window in seconds: "24 hours", "30 minutes", "90 seconds". */
